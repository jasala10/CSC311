...
Phase 1, Step 1:
BP vs FA with Quadratic Learning Task
...
B = rand(n_neurons(n_layers), n_neurons(1)) - 0.5;  
    %   (for quadratic target fn)
    %   note the dimensions of B. A row for every output neuron, a 
    % column for every input neuron. Rand assigns a real number from 0-1
    % and we subtract 0.5 to normalize it to mean 0
...
[W_FA, dL_dW_FA, b_FA, dL_db_FA, y_FA, ...
    dL_dv_FA, WB_FA] = deal(cell(n_layers, 1));
...
% ~~~ SET UP LAYERS ~~~ %
    ...
    for t = 0:Dt:dur
        ...
        for l = n_layers - 1:-1:2
            ...
            if any(isnan(dL_dW_FA{l})) | any(isinf(dL_dW_FA{l}))
                disp(['NaN or Inf in dL_dW_FA at layer ', num2str(l)]);
            end
        end
    ...
    title('Feedback Alignment versus Backpropagation for Quadratic Target function')
    
    subplot(2, 1, 1);
    loglog(DATA(1, :), DATA(2, :), 'Color', [1, 0.2, 0.5]);  % NSE for Feedback Alignment (log scale)
    ylabel('\it{NSE (FA)}');
...
% figure;
% histogram(error_vec, 30); % 30 bins for the histogram (adjust as needed)
% xlabel('Error Difference (FA - BP)');
% ylabel('Frequency');
...
% figure;
% histogram(error_vec, 30); % 30 bins for the histogram (you can adjust this number)
% xlabel('Error Difference (FA - BP)');
% ylabel('Frequency');
% title('Distribution of Error Differences');
